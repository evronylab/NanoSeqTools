...
#' @param BSgenomecontigs A vector of numeric indices of the contigs of the BSgenome package from which to calculate the genome trinucleotide background (e.g., 1:24 for BSgenome.Hsapiens.UCSC.hg38, or 1:21 for BSgenome.Mmusculus.UCSC.mm10). This should match the contigs analyzed by the main NanoSeq pipeline.
...
load_nanoseq_data <- function(dirs, sample_names, BSgenomepackagename, BSgenomecontigs, exclude_regions = NULL, tabix_bin, tabix_threads = 1) {
  ...
  if(length(dirs) != length(sample_names)){
   ...
   # Remove strand information if present in exclude_regions and export regions to BED file
  ...
  for(i in 1:length(dirs)){
    ...
    if(!is.null(exclude_regions)){
    	...
    	system(paste(tabix_bin,"-@",tabix_threads,"-R",tmp.exclude_regions,paste0(dir,"/results.cov.bed.gz"),"| sed 's/;/\t/g' | awk 'BEGIN{OFS=\"\t\"}{print $1,$2,$3,$6,$4,$5}' >",tmp.bedcov.exclude_regions))
  ...
  if(!is.null(exclude_regions)){
  	invisible(file.remove(tmp.exclude_regions))
  }
  ...
  results <- list(
    ...
    dirs = dirs,
  	...
  	BSgenomecontigs = BSgenomecontigs,
