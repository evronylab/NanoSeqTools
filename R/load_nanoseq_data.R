...
#'
#' @param dirs List of all the directories containing NanoSeq results to load.
#' @param suffix_to_remove A string to remove from the NanoSeq results directory names, to retain only the Sample IDs without any suffixes. If there is no suffix to remove, give the input as "NA" (default).
...
load_nanoseq_data <- function(dirs, suffix_to_remove = "NA", BSgenomepackagename, BSgenomecontigs) {
  ...
  for (dir in dirs) {
    ...
    # Obtain sample name by removing suffix_to_remove from directory name
    if (suffix_to_remove != "NA") {
      sample_name <- gsub(paste0(suffix_to_remove, "$"), "", basename(dir))
    }

    # Load vcf files
    vcf_snp <- read.vcfR(paste0(dir,"/results.muts.vcf.gz"))
    vcf_snp.fix[[sample_name]] <- data.frame(vcf_snp@fix) %>% filter(FILTER == "PASS")
    
    vcf_indel <- read.vcfR(paste0(dir,"/results.indel.vcf.gz"))
