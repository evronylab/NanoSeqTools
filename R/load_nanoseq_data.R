...
#' @param source Data source format to load. Use "nanoseq" for cancerit/NanoSeq output directories or "dupcaller" for AlexandrovLab/DupCaller output directories. Default is "nanoseq".
...
#' @return A list of data files with all the relevant results generated by NanoSeq or DupCaller pipelines.
#' * sample_names: A vector of all sample IDs that were loaded
#' * dirs: A character vector of the directories containing results generated by the NanoSeq or DupCaller pipeline
...
#' * vcf_snp.fix: List (one object per sample) containing the fixed information (fix) from the SNP vcf (only FILTER = PASS mutations). For DupCaller data, multi-base substitutions are split before downstream analysis into one single-base substitution row per altered base.
...
load_nanoseq_data <- function(dirs, sample_names, BSgenomepackagename, genome_contigs, exclude_regions = NULL, bedtools_bin, source = c("nanoseq","dupcaller"), dupcaller_result_prefixes = NULL) {
  ...
  if(source == "dupcaller"){
    ...
    result_paths <- tibble(
      ...
      snv=str_c(dirs,"/",dupcaller_result_prefixes,"_snv.vcf"),
      indel=str_c(dirs,"/",dupcaller_result_prefixes,"_indel.vcf"),
      coverage=str_c(dirs,"/",dupcaller_result_prefixes,"_coverage.bed.gz"),
      trinuc=str_c(dirs,"/",dupcaller_result_prefixes,"_trinuc_by_duplex_group.txt"),
      stats=str_c(dirs,"/",dupcaller_result_prefixes,"_stats.txt"),
      ssc_purine=NA_character_,
      ssc_pyrimidine=NA_character_,
      estimated_error_rates=NA_character_
    )
  }else{
    result_paths <- tibble(
      dir=dirs,
      prefix=NA_character_,
      snv=str_c(dirs,"/results.muts.vcf.gz"),
      indel=str_c(dirs,"/results.indel.vcf.gz"),
      coverage=str_c(dirs,"/results.cov.bed.gz"),
      trinuc=str_c(dirs,"/results.trint_counts_and_ratio2genome.tsv"),
      stats=NA_character_,
      ssc_purine=str_c(dirs,"/results.SSC-mismatches-Purine.triprofiles.tsv"),
      ssc_pyrimidine=str_c(dirs,"/results.SSC-mismatches-Pyrimidine.triprofiles.tsv"),
      estimated_error_rates=str_c(dirs,"/results.estimated_error_rates.tsv")
    )
  }
  ...
  for(i in 1:length(dirs)){
    ...
    sample_paths <- result_paths[i,]
    sample_name <- sample_names[i]

    setTxtProgressBar(pb,i/length(dirs)*100)

    # Load vcf files
    vcf_snp <- read.vcfR(sample_paths$snv,verbose=FALSE)
    vcf_snp.fix[[sample_name]] <- data.frame(vcf_snp@fix) %>%	filter(FILTER == "PASS") %>% mutate(POS=as.numeric(POS))
    if(source == "dupcaller"){
      vcf_snp.fix[[sample_name]] <- split_dupcaller_sbs_records(vcf_snp.fix[[sample_name]],eval(parse(text=BSgenomepackagename)))
    }

    vcf_indel <- read.vcfR(sample_paths$indel,verbose=FALSE)
    ...
    if(source == "dupcaller"){
      ...
      results.trint_counts_and_ratio2genome[[sample_name]] <- read.delim(sample_paths$trinuc,header=FALSE,skip=1) %>%
        dplyr::select(-V3) %>%
    ...
    if(source == "nanoseq"){
      results.SSC_mismatches_purine[[sample_name]] <- read.delim(sample_paths$ssc_purine, header=FALSE) %>%
        mutate(tri = str_c(str_sub(V1,1,4),str_sub(V1,1,1),str_sub(V1,5,5),str_sub(V1,3,3))) %>%
        dplyr::rename(value=V2) %>%
        dplyr::select(tri,value)

      results.SSC_mismatches_pyrimidine[[sample_name]] <- read.delim(sample_paths$ssc_pyrimidine, header=FALSE) %>%
        mutate(tri = str_c(str_sub(V1,1,4),str_sub(V1,1,1),str_sub(V1,5,5),str_sub(V1,3,3))) %>%
        dplyr::rename(value=V2) %>%
        dplyr::select(tri,value)

      results.estimated_error_rates[[sample_name]] <- read.delim(sample_paths$estimated_error_rates, header=FALSE, row.names=1) %>%
        t %>%
        as.data.frame %>%
        remove_rownames
    }
